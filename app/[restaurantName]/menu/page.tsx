...
import { getRestaurantBySlug, getActiveMenuItems, getCurrencySymbol, getCurrencyName, type MenuItem, type Restaurant } from "@/app/data/restaurants";
...
const MenuItemCard = ({ 
  ...
  return (
    ...
    >
      {/* صورة الوجبة - تصغير الصورة للشاشات الكبيرة */}
      ...
      >
        ...
        {/* السعر - تصغير حجم السعر للشاشات الكبيرة */}
        <div 
      ...
      {/* معلومات الوجبة - تقليل المسافات للشاشات الكبيرة */}
      <div className="p-2 sm:p-3 md:p-2.5 lg:p-2 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between gap-2 mt-auto pt-1">
          {/* أزرار التحكم بالكمية - تصغير الأزرار للشاشات الكبيرة */}
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5 flex-shrink-0">
            ...
            >
              <Minus size={12} className="text-gray-600 sm:w-3 sm:h-3 md:w-3 md:h-3" />
            </button>
            ...
            >
              <Plus size={12} className="text-gray-600 sm:w-3 sm:h-3 md:w-3 md:h-3" />
            </button>
          </div>
        </div>

        {/* زر الإضافة للسلة - تصغير الزر للشاشات الكبيرة */}
        <button
        ...
        >
          <ShoppingBag size={12} className="sm:w-3 sm:h-3" />
          <span className="xs:inline text-xs">أضف للطلب</span>
        </button>
      
        {/* مؤشر الوجبة في السلة - تصغير النص للشاشات الكبيرة */}
        {cartQuantity > 0 && (
          <motion.div 
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 px-3 py-3 rounded-full shadow-2xl flex items-center gap-3 text-white font-bold"
        style={{ backgroundColor: primaryColor }}
      ...
      >
        <ShoppingBag size={20} />
        <span>عرض الطلب</span>
        <span className="bg-white/20 px-1 py-0.5 rounded-full text-xs">{totalItems}</span>
      </motion.button>
            ...
            >
              ...
              <div className="px-4 py-2 border-t bg-white">
                ...
                <motion.button
                  ...
                  className="w-full py-3.5 rounded-xl text-white font-bold flex items-center justify-center gap-2 shadow-lg transition-all"
                  style={{ backgroundColor: primaryColor }}
                >
                  <Send size={18} />
                  تأكيد الطلب وإرسال
...
export default function RestaurantMenuPage() {
  ...
  // جلب بيانات المطعم والتحقق من رقم الطاولة
  useEffect(() => {
    const fetchRestaurant = () => {
      const found = getRestaurantBySlug(restaurantName);
      if (!found) {
        notFound();
        return;
      }
      setRestaurant(found);
      const activeItems = getActiveMenuItems(found);
      setMenuItems(activeItems);
      
      // التحقق من رقم الطاولة من URL
      if (tableFromUrl) {
        const tableNum = parseInt(tableFromUrl);
        if (!isNaN(tableNum) && tableNum >= 1 && tableNum <= found.numberOfTables) {
          setTableNumber(tableNum);
        } else {
          // رقم طاولة غير صحيح - إعادة توجيه إلى أول طاولة أو عرض خطأ
          router.push(`/${restaurantName}/menu?table=1`);
          return;
        }
      } else {
        // لا يوجد رقم طاولة في الرابط - إعادة توجيه إلى أول طاولة
        router.push(`/${restaurantName}/menu?table=1`);
        return;
      }
    ...
    fetchRestaurant();
  }, [restaurantName, tableFromUrl, router]);
  ...
  // حساب وقت التحضير المقدر (يعتمد على عدد الأطباق)
  const calculatePreparationTime = (itemsCount: number) => {
    if (!restaurant) return 0;
    // الوقت الأساسي + دقيقتين إضافيتين لكل طبق إضافي بعد الطبق الأول
    return restaurant.averagePreparationTime + Math.max(0, (itemsCount - 1) * 2);
  };

  // إرسال الطلب
  const submitOrder = (globalNotes: string) => {
    ...
    // طباعة الطلب في الكونسول مع البيانات الجديدة
    console.log("========== طلب جديد ==========");
    console.log(`مطعم: ${restaurant.name}`);
    console.log(`رقم الطاولة: ${tableNumber}`);
    console.log(`عملة التسعير: ${getCurrencyName(restaurant.currency)} (${currencySymbol})`);
    console.log(`متوسط وقت التحضير: ${preparationTime} دقيقة`);
    console.log(`رسوم الخدمة: ${restaurant.serviceFee} ${currencySymbol}`);
    console.log(`ملاحظات عامة: ${globalNotes || "لا توجد"}`);
    console.log("الوجبات:");
    cartItems.forEach(item => {
      console.log(`- ${item.name} × ${item.quantity} = ${item.price * item.quantity} ${currencySymbol}`);
      if (item.notes) console.log(`  ملاحظات: ${item.notes}`);
    });
    console.log(`المجموع الفرعي: ${subtotal} ${currencySymbol}`);
    console.log(`الإجمالي (مع رسوم الخدمة): ${totalAmount} ${currencySymbol}`);
    console.log("================================");
  ...
  // استخدام categories من بيانات المطعم
  ...
  const maxTableNumber = restaurant?.numberOfTables || 10;
  const currencySymbol = restaurant ? getCurrencySymbol(restaurant.currency) : "";
  const currencyName = restaurant ? getCurrencyName(restaurant.currency) : "";

  if (loading || tableNumber === null) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center" dir="rtl">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 mx-auto mb-3" style={{ borderColor: primaryColor }} />
